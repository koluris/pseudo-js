pseudo = function() {
  ...
  function prepareExe(resp) {
    ...
    const offset = header[2+4]&(io.ram.ub.bLen-1); // Offset needs boundaries... huh?
    const size   = header[2+5];

    // Set mem
    io.ram.ub.set(new UintBcap(resp, EXE_HEADER_SIZE, size), offset);
  ...
  this.awake = function() {
    ...
    $(function() { // DOMContentLoaded
      ...
      request('bios/scph1001.bin', function(resp) {
        // Move BIOS to Mem
        io.rom.ub.set(new UintBcap(resp));
      });
    });
  };
